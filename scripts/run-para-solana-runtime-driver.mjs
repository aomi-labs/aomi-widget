...
const mode = getArg("mode", "sign");
const timeoutMs = Number(getArg("timeout-ms", "180000"));
...
async function main() {
  ...
  const runId = `para-solana-driver-${Date.now()}`;
  const driverUrl =
    `${baseUrl}/dev/para-solana-runtime-driver` +
