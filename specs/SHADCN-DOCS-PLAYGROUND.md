...
- **Workspace Setup**
  ...
  - Tailwind v4: add a docs entry stylesheet that imports `@aomi-labs/react/styles.css` (or `../../packages/react/src/styles.css` in dev). Avoid parallel Tailwind pipelines; share tokens via the existing `packages/react/src/themes/` files.
