...
flowchart TB
  ...
  subgraph bff["portal BFF · next.js · apps/portal"]
    ...
    TOK["bearer route<br/>/api/bff/auth/token · cross-origin seam"]
  end
...
- **TS `aomi`** (`@aomi-labs/client`, `bin: aomi`) — remote client, **does SIWE login**
  ...
  it does **not** call `/api/bff/auth/token`.
...
(EdDSA mint, 15m), `token.ts` (`/api/bff/auth/token`), `account-graph.ts`
