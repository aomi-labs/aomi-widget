...
function ExtUserProviderImpl({ children }) {
  ...
  const addExtValue = useCallback4(
    (key, value) => {
      setUserState((prev) => {
  ...
  const removeExtValue = useCallback4(
    (key) => {
      setUserState((prev) => {
        const ext = prev.ext;
...
function useWalletHandler({
  ...
  const [hasBlockingWalletRequests, setHasBlockingWalletRequests] = useState5(false);
...
function getConnectedWalletId(userState) {
  var _a;
  return (_a = UserStateHelpers.address(userState)) != null ? _a : UserStateHelpers.svmAddress(userState);
}
...
function useRemoteThreadListSync(context, sessions, remoteThreads) {
  ...
  const connectedAddress = UserStateHelpers.isConnected(user) ? getConnectedWalletId(user) : void 0;
...
function getConnectedWalletId2(userState) {
  var _a;
  return (_a = UserState3.address(userState)) != null ? _a : UserState3.svmAddress(userState);
}
