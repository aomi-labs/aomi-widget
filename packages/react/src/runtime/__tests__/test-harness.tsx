...
vi.mock("@aomi-labs/client", async (importOriginal) => {
  ...
  class MockSession {
    ...
    private _paymentMethod: AomiPaymentMethod | null = null;
    ...
    constructor(
      ...
      this._app = opts?.app;
      this._paymentMethod = opts?.paymentMethod ?? null;
    ...
    async sendAsync(message: string) {
      const response = await this.client.sendMessage(this.sessionId, message, {
        app: this._app,
        publicKey:
          this._publicKey ?? (this._userState?.address as string | undefined),
        apiKey: this._apiKey,
    ...
    syncRuntimeOptions(options: {
      ...
      this._paymentMethod = options.paymentMethod ?? null;
    ...
    private syncWalletRequests() {
      const pendingTxs =
        ...
        !Array.isArray(this._userState.pending_txs)
          ? (this._userState.pending_txs as Record<
              string,
      ...
      const pendingEip712s =
        ...
        !Array.isArray(this._userState.pending_eip712s)
          ? (this._userState.pending_eip712s as Record<
              string,
              Record<string, unknown>
            >)
          : {};
