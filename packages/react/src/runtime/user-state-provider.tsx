...
function getLegacySessionPublicKey(userState: UserState): string | undefined {
  ...
  const evm = Array.isArray(userState.evm) ? userState.evm[0] : undefined;
  if (!address?.startsWith("0x")) {
    return undefined;
  }
  if (UserStateHelpers.chainId(userState) === undefined && !evm?.address) {
    return undefined;
  }
  return address;
}
...
function useWalletStateSync(
      ...
      evm: [
        {
          ...
          ens_name: UserStateHelpers.ensName(nextUser),
          aa: {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onUserStateChange(async (newUser) => {
      ...
      const previousAddress = normalizeWalletId(
        prevWalletState.evm?.[0]?.address,
      );
