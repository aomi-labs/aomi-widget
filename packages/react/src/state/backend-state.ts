...
export function setThreadRunning(state: BackendState, threadId: string, running: boolean) {
  if (running) {
...
export function enqueuePendingChat(state: BackendState, threadId: string, text: string) {
  const existing = state.pendingChat.get(threadId) ?? [];
  state.pendingChat.set(threadId, [...existing, text]);
}

export function dequeuePendingChat(state: BackendState, threadId: string): string[] {
