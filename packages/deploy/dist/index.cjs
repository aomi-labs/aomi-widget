...
__export(index_exports, {
  ...
  COMMIT_RE: () => COMMIT_RE,
  DEFAULT_RELEASE_TAG_CONVENTION: () => DEFAULT_RELEASE_TAG_CONVENTION,
  DEPLOYMENT_DIR: () => DEPLOYMENT_DIR,
  DEPLOYMENT_FILE: () => DEPLOYMENT_FILE,
  DeployError: () => DeployError,
  DeploymentClient: () => DeploymentClient,
  PathScopeError: () => PathScopeError,
  RESERVED_MANIFEST_PATH: () => RESERVED_MANIFEST_PATH,
...
function deriveSourceCommit(files) {
  ...
  return (0, import_node_crypto.createHash)("sha256").update(lines.join("\n")).digest("hex").slice(0, 40);
}
...
function sha256Prefixed(content) {
  return `sha256:${(0, import_node_crypto2.createHash)("sha256").update(toBytes(content)).digest("hex")}`;
}
...
function resolveGitHubApi(opts) {
  ...
  const octokit = new import_rest.Octokit({ auth: opts.github.botPat });
