...
`src/` holds the publishable widget library: `components/assistant-ui/` for the Aomi frame and chat surfaces, `components/ui/` for shadcn-style primitives, `hooks/` for reusable state, `lib/` for runtime/context helpers, `utils/` for wallet helpers, and `themes/` for CSS token packs that feed the `styles.css` entry point. The demo Next.js app lives in `example/` and consumes the built package from `dist/`; use it to validate UI flows before publishing. Static assets shared by the example go in `public/`, while high-level product briefs sit in `specs/`—update both when you introduce new flows.
...
- `pnpm --filter example dev` — launches the demo at http://localhost:3000 using the last built library.
- `pnpm run dev:example:live` — watches `src/` with tsup while running the example so library updates hot-reload into the demo.
...
## Testing Guidelines
There is no dedicated automated test suite yet, so treat `pnpm run build:lib` and `pnpm run lint` as the minimum regression gates. Validate UI behavior through the `example/` app before opening a PR, and capture regressions with story-specific checks or lightweight React Testing Library specs (`*.test.tsx`) colocated with the component whenever you add new logic branches.
