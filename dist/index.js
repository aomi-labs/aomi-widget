...
import { createContext as createContext4, useCallback as useCallback12, useContext as useContext4, useEffect as useEffect10, useRef as useRef8 } from "react";
...
function useThreadLifecycle({
  ...
  const onSwitchToNewThread = useCallback6(async () => {
    var _a;
  ...
  const onSwitchToThread = useCallback6(
    (threadId) => {
      setCurrentThreadId(threadId);
    },
    [setCurrentThreadId]
  );
  const onRename = useCallback6(
  ...
  const onUnarchive = useCallback6(
    async (threadId) => {
      updateThreadMetadata(threadId, { status: "regular" });
      try {
...
function useThreadListSync({
  ...
  const threadMetadataRef = useRef4(threadMetadata);
  const threadCntRef = useRef4(threadCnt);
  useEffect6(() => {
...
function useThreadMessageStore({
  ...
  const applySessionMessagesToThread = useCallback7(
    (threadId, msgs) => {
      var _a, _b;
      if (!msgs) return;
  ...
  const applyMessagesForThread = useCallback7(
    (threadId, msgs) => {
...
function useThreadMessaging({
  ...
  const sendSystemMessageNow = useCallback8(
  ...
  const flushPendingSystemMessages = useCallback8(
    async (threadId) => {
  ...
  const ensureBackendSessionForThread = useCallback8(
    (threadId) => {
  ...
  const sendSystemMessage = useCallback8(
    async (message) => {
      if (!isThreadReady(currentThreadId)) {
...
function useThreadPolling({
  ...
  const pollingThreadIdRef = useRef5(null);
  const stopPolling = useCallback9(() => {
  ...
  const startPolling = useCallback9(() => {
  ...
  const interruptThread = useCallback9(
    async (threadId) => {
      if (!isThreadReady(threadId)) return;
      const backendThreadId = resolveThreadId(threadId);
      try {
  ...
  useEffect7(() => stopPolling, [stopPolling]);
  return {
    interruptThread,
    isRunning,
    setIsRunning,
...
function useThreadUpdates({
  ...
  const [subscribableSessionId, setSubscribableSessionId] = useState9(null);
  const [updateSubscriptionsTick, setUpdateSubscriptionsTick] = useState9(0);
  const lastEventIdBySessionRef = useRef6(/* @__PURE__ */ new Map());
  const eventsInFlightRef = useRef6(/* @__PURE__ */ new Set());
  const updateSubscriptionsRef = useRef6(/* @__PURE__ */ new Map());
  const bumpUpdateSubscriptions = useCallback10(() => {
    setUpdateSubscriptionsTick((prev) => prev + 1);
  }, []);
  const applyTitleChanged = useCallback10(
    (sessionId, newTitle) => {
  ...
  const drainEvents = useCallback10(
    async (sessionId) => {
      var _a;
  ...
  const ensureUpdateSubscription = useCallback10(
    (sessionId) => {
  ...
  const removeUpdateSubscription = useCallback10((sessionId) => {
...
function useWalletTx({
  ...
  const handledWalletTxRequestsRef = useRef7(/* @__PURE__ */ new Set());
  const walletTxQueueRef = useRef7([]);
  const walletTxInFlightRef = useRef7(false);
  const enqueueWalletTxRequest = useCallback11(
  ...
  const drainWalletTxQueue = useCallback11(async () => {
    var _a, _b, _c;
  ...
  const handleWalletTxRequest = useCallback11(
    (sessionId, threadId, request) => {
...
function AomiRuntimeProvider({
  ...
  const currentThreadIdRef = useRef8(currentThreadId);
  useEffect10(() => {
    currentThreadIdRef.current = currentThreadId;
  }, [currentThreadId]);
  const pendingChatMessagesRef = useRef8(/* @__PURE__ */ new Map());
  const pendingSystemMessagesRef = useRef8(/* @__PURE__ */ new Map());
  const skipInitialFetchRef = useRef8(/* @__PURE__ */ new Set());
  const creatingThreadIdRef = useRef8(null);
  const createThreadPromiseRef = useRef8(null);
  ...
  const onCancel = useCallback12(async () => {
  ...
  return /* @__PURE__ */ jsx20(RuntimeActionsContext.Provider, { value: { sendSystemMessage: messaging.sendSystemMessage }, children: /* @__PURE__ */ jsx20(AssistantRuntimeProvider, { runtime, children }) });
}
...
function WalletSystemMessageEmitter({ wallet }) {
  ...
  const { showNotification } = useNotification();
  const lastWalletRef = useRef9({ isConnected: false });
  useEffect11(() => {
...
var AomiFrame = ({
  ...
  const [wallet, setWalletState] = useState10({
    isConnected: false,
    address: void 0,
    chainId: void 0,
    ensName: void 0
  });
  const setWallet = useCallback13((data) => {
