...
- Onboarding product choices are code constants behind `launchConfig()`: public default platform `community` via `resolveDeployPlatform()`, explicit white-labeled partner override with `NEXT_PUBLIC_DEPLOY_PLATFORM` such as `somm.finance`, template repo `aomi-labs/playground-example`, source branch `main`, manifest path `aomi.toml`, public created repos, and no activation `target_tags`.
