...
| `POST /api/onboard/dry-run`        | Portal onboarding deploy preview step                       | Rust backend `POST /api/platforms/:platform/sources/sync-installed` when needed, then `POST /api/platforms/:platform/deploy` with `dry_run: true`  | Uses a cached app source id or syncs the repo, then asks the backend to plan a deployment without committing/activating it.              | `apps/portal/src/app/api/onboard/dry-run/route.ts`        |
| `POST /api/onboard/deploy`         | Portal onboarding deploy step                               | Rust backend `POST /api/platforms/:platform/sources/sync-installed` when needed, then `POST /api/platforms/:platform/deploy` with `dry_run: false` | Uses a cached app source id or syncs the repo, then starts the backend deployment flow, returning deployment id, apps, and release tags. | `apps/portal/src/app/api/onboard/deploy/route.ts`         |
