...
- Launch product choices are env-driven server-side config: `APP_DEPLOY_PLATFORM` (default `community`), `APP_DEPLOY_AOMI_TOML_PATHS` (default `aomi.toml`, comma-separated), and optional `APP_DEPLOY_TARGET_TAGS` (comma-separated); the deploy source ref is an immutable commit SHA from `APP_DEPLOY_SOURCE_REF` (or `APP_DEPLOY_SOURCE_COMMIT`). Client display/install context may use `NEXT_PUBLIC_AOMI_DEPLOY_PLATFORM`.
