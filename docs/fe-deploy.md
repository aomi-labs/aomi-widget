...
> - Launch config is server-env-driven: `APP_DEPLOY_PLATFORM`, optional `APP_DEPLOY_SOURCE_BRANCH`, `APP_DEPLOY_AOMI_TOML_PATHS`, and optional `APP_DEPLOY_TARGET_TAGS`. `APP_DEPLOY_SOURCE_REF` / `APP_DEPLOY_SOURCE_COMMIT` are optional immutable commit pins; otherwise the backend resolves the configured source branch, or the repo default branch when none is configured, with the GitHub App installation token. Portal deploy pins the preflight-resolved SHA when committing.
