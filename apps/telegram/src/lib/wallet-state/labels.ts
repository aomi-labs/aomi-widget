...
function opLabel(state: UserWalletState): string | null {
  ...
  if (op.status === "awaiting_wallet") {
    ...
    if (op.kind === "switch_network")
      return "Awaiting wallet approval for network switch";
    if (op.kind === "sign_tx")
      return "Awaiting wallet approval for transaction";
    if (op.kind === "sign_eip712")
      return "Awaiting wallet approval for signature";
  }
  ...
  if (op.status === "failed")
    return `Wallet action failed${op.errorCode ? ` (${op.errorCode})` : ""}`;
...
export function deriveWalletStateLabel(state: UserWalletState): string {
  ...
  if (state.presence === "connecting")
    return "Awaiting wallet connection approval";
