...
export async function GET(request: NextRequest) {
  ...
  if (!eip712Id && !sessionKey) {
    return NextResponse.json({ error: 'Missing eip712_id or session_key' }, { status: 400 });
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const description = typeof body?.description === 'string' ? body.description : 'EIP-712 signature request';
    const eip712Id = typeof body?.eip712_id === 'string' ? body.eip712_id : undefined;
    const pendingEip712Id = typeof body?.pending_eip712_id === 'number'
      ? body.pending_eip712_id
      : (typeof body?.pendingEip712Id === 'number' ? body.pendingEip712Id : undefined);

    if (!sessionKey || !typedData) {
      return NextResponse.json({ error: 'Missing session_key or typed_data' }, { status: 400 });
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (!eip712Id || !status) {
      return NextResponse.json({ error: 'Missing eip712_id or status' }, { status: 400 });
    }
    ...
    if (status === 'awaiting_wallet') {
      ...
      return NextResponse.json({ success: true, eip712Id, status, state: found.state, label: found.state.label });
    }

    if (status === 'signed') {
      const found = markOperationSuccess(eip712Id, { signature });
      if (!found) return NextResponse.json({ error: 'Request not found' }, { status: 404 });
      return NextResponse.json({ success: true, eip712Id, status, signature, state: found.state, label: found.state.label });
    }
