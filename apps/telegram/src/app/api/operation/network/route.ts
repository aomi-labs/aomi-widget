...
export async function GET(request: NextRequest) {
  ...
  if (!switchId && !sessionKey) {
    return NextResponse.json(
      { error: "Missing switch_id or session_key" },
      { status: 400 },
    );
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const switchId =
      typeof body?.switch_id === "string" ? body.switch_id : undefined;

    if (!sessionKey || !Number.isFinite(chainId) || chainId <= 0) {
      return NextResponse.json(
        { error: "Missing session_key or chain_id" },
        { status: 400 },
      );
    }
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const chainId =
      typeof body?.chain_id === "number" ? body.chain_id : undefined;
    const address =
      typeof body?.address === "string" ? body.address : undefined;

    if (!switchId || !status) {
      return NextResponse.json(
        { error: "Missing switch_id or status" },
        { status: 400 },
      );
    }
