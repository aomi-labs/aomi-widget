...
test("starting a new operation preempts the current one", () => {
  ...
  const oldUpdate = markOperationSuccess(first.operationId, {
    txHash: "0xdead",
  });
  assert.equal(oldUpdate, undefined);
});

test("sign operation can store ERC-191 non-typed data", () => {
  const { operationId } = startOperation("u1", "sign_eip712", {
    sessionKey: "telegram:dm:123",
    nonTypedData: "Sign in with Ethereum",
  ...
  const active = getWalletState("u1").activeOperation;
  assert.ok(active);
  assert.equal(active?.operationId, operationId);
  assert.equal(active?.kind, "sign_eip712");
  assert.equal(active?.metadata.nonTypedData, "Sign in with Ethereum");
});
