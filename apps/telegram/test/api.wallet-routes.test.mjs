...
test('tx route: create -> awaiting_wallet -> signed', async () => {
  const createRes = await txRoute.POST(jsonReq({
  ...
  const getPending = await txRoute.GET(getReq(`http://localhost/api/operation/tx?tx_id=${encodeURIComponent(txId)}`));
  const pendingJson = await getPending.json();
  assert.equal(pendingJson.pending, true);
  assert.equal(pendingJson.status, 'pending');

  const awaitingRes = await txRoute.PUT(jsonReq({ tx_id: txId, status: 'awaiting_wallet' }));
  assert.equal(awaitingRes.status, 200);

  const awaitingGet = await txRoute.GET(getReq(`http://localhost/api/operation/tx?tx_id=${encodeURIComponent(txId)}`));
  const awaitingJson = await awaitingGet.json();
  assert.equal(awaitingJson.status, 'awaiting_wallet');

  const signedRes = await txRoute.PUT(jsonReq({ tx_id: txId, status: 'signed', tx_hash: '0xdeadbeef' }));
  assert.equal(signedRes.status, 200);

  const signedGet = await txRoute.GET(getReq(`http://localhost/api/operation/tx?tx_id=${encodeURIComponent(txId)}`));
...
test('eip712 route: create -> rejected', async () => {
  const createRes = await eip712Route.POST(jsonReq({
  ...
  const rejectRes = await eip712Route.PUT(jsonReq({
    eip712_id: eip712Id,
    status: 'rejected',
  ...
  const getRes = await eip712Route.GET(getReq(`http://localhost/api/operation/eip712?eip712_id=${encodeURIComponent(eip712Id)}`));
...
test('network route: create -> switched and session_key lookup resolves active op', async () => {
  const createRes = await networkRoute.POST(jsonReq({
  ...
  const bySession = await networkRoute.GET(getReq('http://localhost/api/operation/network?session_key=telegram%3Adm%3A1003'));
  ...
  const switchedRes = await networkRoute.PUT(jsonReq({
    switch_id: switchId,
    status: 'switched',
    chain_id: 8453,
    address: '0x123',
  ...
  const getSwitched = await networkRoute.GET(getReq(`http://localhost/api/operation/network?switch_id=${encodeURIComponent(switchId)}`));
