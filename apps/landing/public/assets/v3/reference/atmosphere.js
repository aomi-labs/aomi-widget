(() => {
  ...
  atmosphere.id = "v3-atmosphere";
  atmosphere.setAttribute("aria-hidden", "true");

  const toneMap = document.createElementNS("http://www.w3.org/2000/svg", "svg");
  toneMap.classList.add("v3-atmosphere-filters");
  toneMap.innerHTML = `
    <defs>
      <filter id="v3-muted-brand-blue" color-interpolation-filters="sRGB">
  ...
  const createScene = ({ position, section, progress }) => {
    ...
    scene.className = `v3-atmosphere-scene v3-atmosphere-scene--${position}`;
    scene.dataset.v3Anchor = section;
    scene.dataset.v3AnchorProgress = String(progress);
  ...
  const revealWhenReady = () => {
    ...
    if (initializedScenes.length > 0) {
      document.body.classList.add("v3-atmosphere-ready");
      return true;
    }
    return false;
  };
  ...
  if (typeof window.UnicornStudio?.init === "function") {
    ...
    runtime.async = true;
    runtime.dataset.v3UnicornRuntime = "true";
