...
description: Build your own chat interface using headless hooks.
---
...
`isRunning` is derived from the backend `TurnState`. `isSubmitting` covers only
the interval before the first Event exists. Messages update as `MessageEvent`
revisions arrive in the same ordered session snapshot.

```tsx
import { useAomiRuntime } from "@aomi-labs/react";

function StreamMonitor() {
  const { events, turnState, isSubmitting } = useAomiRuntime();
  const latest = events.at(-1);
  return <div>{isSubmitting ? "Submitting" : turnState}: {latest?.type}</div>;
}
```
