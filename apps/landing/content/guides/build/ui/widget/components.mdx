...
The package-level `AomiWidget` is the default integration. The components below
are available for advanced package composition or as source-owned entries in
the Aomi shadcn registry.
...
The complete consumer boundary with Portal auth/account transport, wallet
providers, chains, and execution defaults.
...
For copied source, install `https://aomi.dev/r/aomi-widget.json`.

### AomiFrame

The lower-level chat shell with sidebar, thread management, and runtime wiring.
It does not configure embedded auth or canonical account transport by itself.
...
Displays conversation threads with create, rename, archive, and restore actions.
