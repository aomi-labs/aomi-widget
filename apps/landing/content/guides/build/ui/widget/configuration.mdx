...
# Application whose backend-owned execution policy governs widget writes
...
## Standalone authenticated widget

An unrelated site can mount the packaged widget using an authentication-only
configuration. Importing the provider subpath registers only that provider;
the providerless entry does not bundle Para or Privy. The application ID selects
the server-owned ERC-4337, sponsorship, and fee policy.
...
Portal verifies the provider credential and returns a short-lived, origin-bound
widget session token. The token stays in memory, requests use
`credentials: "omit"`, and sign-out revokes it. Provider tenant material and
Alchemy policy configuration are registered server-side for the application;
they are not widget execution inputs. Configure the consumer origin in the
provider console. Production consumers must use HTTPS; local development may
use `http://localhost`.

Use `auth={{ kind: "browser_wallet" }}` for signed external-wallet login.
Browser, Para, and Privy modes all resolve to the same wallet-binding and
backend-owned sponsored execution contract.
...
function MyComponent() {
  ...
  // Canonical ordered Events and Actions
  console.log(api.events, api.turnState);
  await api.executeAction(api.pendingActions[0].id);
}
```
...
| --------------------- | ------------ | ---------------------- |
| `/v1/agent/chat`    | POST         | Send a message         |
| `/v1/agent/chat/{sessionId}`   | GET          | Poll for state updates |
