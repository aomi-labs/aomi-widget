...
An unrelated site can mount the packaged widget with its own browser-visible
Para project key. Importing the provider subpath registers only that provider;
the providerless entry does not bundle Para or Privy.
...
The consumer key selects the Para tenant and environment; it is not an Aomi
integration key or a server secret. Portal verifies the provider JWT and returns
a short-lived, origin-bound widget session token. The token stays in memory,
requests use `credentials: "omit"`, and sign-out revokes it. Configure the
consumer origin in the Para Developer Portal. Production consumers must use
HTTPS; local development may use `http://localhost`.

Omit `auth` to use signed external-wallet login (SIWE or SIWS) instead. Both
modes resolve to the same Aomi account and normal account/thread routes.
