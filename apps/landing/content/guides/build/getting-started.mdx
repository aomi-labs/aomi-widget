...
For the default Para-backed wallet flow, also add:
...
Advanced consumers can replace the default wallet implementation with their own
adapter by wrapping the widget in `AomiAdapterProvider` or passing an `adapter`
prop to `AomiFrame`.
...
- [Apps](/docs/build/namespaces) -- Understand how API keys and apps work.
