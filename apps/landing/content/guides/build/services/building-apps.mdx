...
Models can be switched at runtime via the `/api/session/model` endpoint without redeploying the app.
...
flowchart TB
    ...
    subgraph "Runtime"
        REQ[POST /api/thread/chat?app=mycoindex] --> LOOKUP[Lookup app]
