...
const response = await fetch("/api/thread/chat?app=mycoindex", {
  method: "POST",
  headers: {
...
const state = await fetch(
  `/api/thread/state?session_id=${sessionId}`
).then((r) => r.json());
...
const events = new EventSource(
  `/api/thread/updates?session_id=${sessionId}`,
);
...
- [API Reference](/docs/build/services/api-reference) -- full endpoint documentation for session operations.
- [Apps and Authentication](/docs/build/namespaces) -- how sessions relate to apps and API keys.
- [Configuration](/docs/build/ui/widget/configuration) -- customize session behavior and thread management in the widget.
