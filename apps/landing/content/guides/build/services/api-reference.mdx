...
```
GET /api/thread/state?session_id={session_id}
```
...
```
POST /api/thread/interrupt?session_id={session_id}
```
...
- [Apps and Authentication](/docs/build/namespaces) -- API key scoping and auth flow.
