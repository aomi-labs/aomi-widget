...
Aomi widgets now expose wallet behavior through `AomiAdapterProvider`. The
default registry install ships with a Para-backed adapter for connection,
account management, transaction execution, typed-data signing, and chain
switching. Advanced consumers can still replace that adapter with their own
wallet stack.
...
the default adapter files and their dependencies are already included. Use the
setup below only when you are customizing the wallet provider layer directly.
