...
Every Aomi-powered assistant runs inside an **App** (internally called a "namespace" in the API). This page explains how Apps, API keys, and sessions work together to provide isolated, authenticated access to your AI assistant.
...
Include the API key in the `X-API-Key` header on every request. The `namespace` query parameter specifies which App to use:
...
> **Note:** The API uses `namespace` as the query parameter for historical reasons. In docs and UI, we refer to this as the "App".
...
3. Backend checks if the key authorizes the requested App (from the `?namespace=` query parameter).
