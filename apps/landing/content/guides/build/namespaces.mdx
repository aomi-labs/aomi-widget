...
```bash
curl -X POST https://api.aomi.dev/api/thread/chat?app=mycoindex \
...
sequenceDiagram
    ...
    Client->>Backend: POST /api/thread/chat?app=mycoindex<br/>X-API-Key: sk-mcd-...<br/>X-Session-Id: session-uuid
