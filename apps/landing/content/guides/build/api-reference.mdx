...
| `X-API-Key` | Your Aomi API key (required for non-default namespaces) |
...
```
POST /api/chat?namespace={namespace}
```
...
| `X-API-Key` | For non-default namespaces | Your API key |
...
Get the namespaces your API key is authorized to access.
...
```ts
const response = await fetch("/api/chat?namespace=mycoindex", {
  method: "POST",
  headers: {
...
| `403` | Forbidden (API key does not authorize this namespace) |
| `404` | Not found (session or resource does not exist) |
| `500` | Internal server error |

## Next Steps

- [Sessions](/docs/core-concepts/sessions) -- how sessions and threads are managed.
- [Namespaces and Authentication](/docs/core-concepts/namespaces) -- API key scoping and auth flow.
