...
**4. Deployed App** — Aomi compiles and deploys the configured Aomi App as an isolated app on our hosted platform. Each app gets a scoped API key and a dedicated chat endpoint. Deployment and scaling are managed by Aomi.

**5. Client Integration** — You are responsible for your own product surface.
Use the complete `AomiWidget`, the headless runtime, or Telegram. Browser
widgets point at Portal, which owns auth, canonical accounts, and authenticated
backend proxying. We provide assistance throughout the integration process.
...
Aomi compiles and deploys the configured Aomi App as an isolated **app** on our hosted platform. An app bundles tools, preamble, model, and credentials into a single addressable endpoint.

```
App:        "mycoindex"
Endpoint:   POST /api/thread/chat?app=mycoindex
...
Each app is fully isolated. MyCoinDex's tools, preamble, and configuration do not affect other apps on the platform. Deployment and scaling are managed by Aomi — you receive a scoped API key and a dedicated chat endpoint.
...
| ------------------- | ---------------------------------------------------- | ----------------------------------------------------- |
| Complete Widget     | `npm install @aomi-labs/widget-lib`                  | Authenticated Para, Privy, or SIWE integration        |
| Source-owned Widget | `npx shadcn add https://aomi.dev/r/aomi-widget.json` | Bespoke component-level customization                 |
| Headless Library    | `npm install @aomi-labs/react`                       | Custom designs and host-owned auth/wallet composition |
| Telegram Bot        | Managed by Aomi                                      | Reaching users without frontend deployment            |

For a cross-origin web integration, register the product origin in Portal's
`AOMI_TRUSTED_ORIGINS`; prefer an exact origin, or use a narrow `*` pattern for
changing preview hosts. Pass the Portal origin to `AomiWidget.apiUrl`;
do not point authenticated browser widgets at the raw backend or add a second
BetterAuth instance to the product app. See the
[Quickstart](/docs/build/quickstart) for Para, Privy, and providerless examples.

## Next Steps

- [Agentic Application](/docs/agentic-application) — how Apps and API keys fit together.
