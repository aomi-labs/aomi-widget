...
sequenceDiagram
    ...
    Frontend->>Aomi: POST /v1/agent/chat?app=mycoindex<br/>X-API-Key: sk-mcd-...<br/>X-Session-Id: session-uuid<br/>{"text": "What's my ETH position worth?"}
...
2. **Frontend sends HTTP POST** to `/v1/agent/chat?app=mycoindex` with the API key and session ID.
