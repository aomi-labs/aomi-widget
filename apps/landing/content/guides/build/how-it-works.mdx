...
sequenceDiagram
    ...
    Frontend->>Aomi: POST /api/thread/chat?app=mycoindex<br/>X-API-Key: sk-mcd-...<br/>X-Session-Id: session-uuid<br/>{"text": "What's my ETH position worth?"}
...
2. **Frontend sends HTTP POST** to `/api/thread/chat?app=mycoindex` with the API key and session ID.
...
- [Apps and Authentication](/docs/build/namespaces) -- how API keys, apps, and sessions fit together.
