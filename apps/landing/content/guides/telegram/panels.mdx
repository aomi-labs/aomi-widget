...
When you send a slash command like `/start` or `/namespace`, the bot responds with a message containing inline keyboard buttons. Tapping a button navigates you to another panel or performs an action. The message updates in place, so the conversation stays clean.
...
- **Namespace** -- Switch AI assistant app
...
Select which AI assistant namespace to use. Each namespace provides different tools and capabilities.

Displays all authorized namespaces as buttons. The currently active namespace is highlighted. Tapping a namespace switches the AI context immediately.
