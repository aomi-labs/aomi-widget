---
title: Building Apps
description: How AomiApps are built with tools, preambles, and model selection.
---

# Building Apps

This page explains how AI assistants are constructed on the Aomi platform. It covers the AomiApp abstraction, the builder pattern, tool definitions, model selection, and namespace registration.
...
Under the hood, tools implement the `AomiTool` trait:

| Field | Type | Description |
| --- | --- | --- |
| `NAME` | `&str` | Tool name as seen by the LLM |
| `NAMESPACE` | `&str` | Which namespace this tool belongs to |
| `Args` | struct | Typed parameters (derived from `JsonSchema`) |
| `run()` | async fn | The function that executes when the tool is called |
...
Once an app is built, it is registered in the backend's namespace map:
...
When a request arrives with `?namespace=mycoindex`, the backend looks up the namespace in this map and routes the request to the corresponding app.

### The AomiBackend Trait

Every registered app implements the `AomiBackend` trait:
...
This trait provides a uniform interface for the backend to interact with any app, regardless of its internal implementation.
...
flowchart TB
    ...
    subgraph "Runtime"
        REQ[POST /api/chat?namespace=mycoindex] --> LOOKUP[Lookup namespace]
...
- [How It Works](/docs/core-concepts/how-it-works) -- see the full request flow with sequence diagrams.
- [API Reference](/docs/core-concepts/api-reference) -- the HTTP endpoints your frontend uses.
- [SDK](/docs/reference/sdk) -- detailed Rust SDK reference for `CoreAppBuilder` and related types.
