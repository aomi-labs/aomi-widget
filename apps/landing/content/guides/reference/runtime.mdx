---
title: Runtime
description: Architecture of the Aomi hosted runtime — session management, namespace isolation, and the reconciliation pipeline.
---

# Aomi Runtime

The Aomi Runtime is the hosted execution environment that powers all Aomi Apps. It manages sessions, routes requests to the correct namespace, handles wallet binding, and orchestrates LLM completion and tool execution. This page describes the architecture at a level relevant to integration partners and developers building on the platform.

## Architecture

The runtime is designed around stateless dispatch and application-defined execution. No request carries server-side session state — every call is routed purely by its credentials and namespace. The application layer owns the definition of what an Aomi App is: which tools it has, how it behaves, and what model it runs on. The runtime underneath is a shared execution environment that treats each app as an opaque unit — it schedules tool calls, manages memory, and forwards LLM completions without needing to understand the domain logic above it. This separation means the same runtime can host a DeFi trading agent and a GameFi assistant side by side with zero coupling between them.
...
- A **backend registry** keyed by namespace and model selection
- An **auth snapshot** per session tracking the last-known API key, public key, and namespace
- **Reconcile locks** to prevent race conditions during session recovery

When a user switches namespaces or connects a different wallet mid-session, the Reconstructor hot-swaps the backend on the live session without creating a new one.
...
**1. Fast path** — The session is already in memory. No database call, no LLM initialization. The runtime verifies auth, reconciles any namespace or wallet changes, and returns the session state.
...
**3. Create** — No session exists. The Reconstructor creates a fresh session, binds it to the requested namespace and model, associates the user's wallet if provided, and inserts it into both memory and the database.
...
Each Aomi App runs as an isolated namespace. A namespace bundles a specific backend implementation with its own set of tools, system prompt, and model configuration. Namespaces do not share state — a DeFi namespace cannot access tools or history from a Prediction namespace.

The runtime initializes all configured namespace backends at startup. When a session is created or switched, the Reconstructor resolves the correct backend from the registry and binds it to the session.
...
- [Integration Guide](/docs/build/integration-guide) — how your APIs connect to the runtime through namespaces
