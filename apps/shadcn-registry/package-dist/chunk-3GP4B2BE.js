...
function createAomiBackendAccountClient(input) {
  ...
  const request = (path, init) => fetchJson(fetchImpl, urlFor(path), init);
  return {
    ...
    updateWallet: (walletId, body) => request(endpoints.walletPath(walletId), {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body)
    }),
    updateAuthIdentity: (identityId, body) => request(endpoints.identityPath(identityId), {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body)
    }),
    unlinkWallet: (walletId) => request(endpoints.walletPath(walletId), { method: "DELETE" }),
    unlinkAuthIdentity: (identityId) => request(endpoints.identityPath(identityId), { method: "DELETE" }),
    createSiweNonce: (body) => request(endpoints.siweNoncePath, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body)
    }),
    verifySiwe: (body) => request(endpoints.siweVerifyPath, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(body)
    })
...
function formatAccountRequestError(status, code) {
...
function useAomiBackendAccountRuntime(input) {
  const accountClient = useMemo3(
    () => createAomiBackendAccountClient({ baseUrl: input.baseUrl }),
    [input.baseUrl]
  );
  const authMessageConfig = useMemo3(
  ...
  const siweInFlight = useRef2(null);
  const accountCreateInFlight = useRef2(null);
  const signedOutEvmKey = useRef2(null);
  const signedOutCredentialKey = useRef2(null);
  const credentialInFlight = useRef2(null);
  const credentialExchanged = useRef2(null);
  const providerSessionAttempted = useRef2(null);
  const credentialFailed = useRef2(null);
  ...
  useEffect4(() => {
    if (!input.enabled || status === "error" || account === null || account.user || !hasAuthMessageConfig(authMessageConfig)) {
      return;
    }
  ...
  useEffect4(() => {
    if (!input.enabled || status === "error" || input.auth.status !== "authenticated") {
      return;
    }
  ...
  return {
    ...
    linkWallet: async (wallet) => {
      if (wallet.family !== "evm" || !input.evm.signMessageForAccount && !input.evm.signMessageAsync) {
...
var publicEnv = typeof process === "undefined" ? void 0 : {
  NODE_ENV: process.env.NODE_ENV,
  NEXT_PUBLIC_AA_PROVIDER: process.env.NEXT_PUBLIC_AA_PROVIDER,
  NEXT_PUBLIC_ALCHEMY_API_KEY: process.env.NEXT_PUBLIC_ALCHEMY_API_KEY,
  NEXT_PUBLIC_ALCHEMY_GAS_POLICY_ID: process.env.NEXT_PUBLIC_ALCHEMY_GAS_POLICY_ID,
  NEXT_PUBLIC_AOMI_WC_PROJECT_ID: process.env.NEXT_PUBLIC_AOMI_WC_PROJECT_ID,
  NEXT_PUBLIC_BACKEND_URL: process.env.NEXT_PUBLIC_BACKEND_URL,
  NEXT_PUBLIC_FULL_TESTNET_RPC_MAP: process.env.NEXT_PUBLIC_FULL_TESTNET_RPC_MAP,
...
// src/lib/wallet-kit/execution/aa-provider-state.ts
import {
  createAAProviderState
} from "@aomi-labs/client";

// src/lib/wallet-kit/execution/aa-owner.ts
function toClientAAOwner(owner) {
  switch (owner.kind) {
...
function resolveRequestedAAMode(payload, isBatch, preferAAForSingleCall = false, forceAA = false) {
...
function normalizeAtomicCapabilities(raw) {
  ...
  for (const [key, value] of Object.entries(raw)) {
    ...
    const record = value;
    normalized[key] = record.atomic?.status === "ready" ? {
      ...record,
      atomic: {
        ...record.atomic,
        status: "supported"
...
function buildAaAttempts(aaRequestedMode, shouldUseExternalSigner, allowedModes) {
  const allowed = allowedModes?.length ? new Set(allowedModes) : null;
  const filterAllowed = (attempts) => allowed ? attempts.filter((attempt) => allowed.has(attempt.requestedMode)) : attempts;
...
function hasResolvedAAProviderState(providerState) {
...
async function executeWalletKitTransaction({
  ...
  resolveAAProviderState: resolveAAProviderState2,
  ...
  const requiresSponsoredExecution = state.nativeWalletExecution?.sponsorship?.mode === "required";
  ...
  const executeWithProviderState = async (providerState) => executeWalletCalls({
    callList,
    ...
    providerState,
    sendCallsSyncAsync: state.sendCallsSyncAsync ? async (args) => {
    ...
    switchChainAsync: state.switchChainAsync ? async ({ chainId }) => {
  ...
  const aaStateResolver = resolveAAProviderState2;
  const aaAttempts = aaPolicy !== "off" && aaStateResolver ? buildAaAttempts(aaRequestedMode, shouldUseExternalSigner, aaModes) : [];
  if (aaAttempts.length === 0) {
    ...
    for (const attempt of aaAttempts) {
      ...
      try {
        ...
        if (aaRequestedMode === "7702" && attempt.requestedMode === "4337" && !finalFallbackReason) {
    ...
    if (!execution) {
      ...
      console.warn(
        ...
        {
          error: lastAAError instanceof Error ? lastAAError.message : String(lastAAError ?? "unknown")
        }
      );
      execution = await executeWithProviderState(DISABLED_PROVIDER_STATE);
  ...
  return {
    ...
    aaRequestedMode,
    aaResolvedMode,
    aaFallbackReason: finalFallbackReason ?? (aaRequestedMode === "7702" && aaResolvedMode === "4337" ? "requested_7702_fallback_4337" : aaRequestedMode !== "none" && aaResolvedMode === "none" ? "aa_unavailable_fallback_eoa" : void 0),
...
// src/lib/wallet-kit/execution/aa-provider-state.ts
var ALCHEMY_API_KEY = publicEnv?.NEXT_PUBLIC_ALCHEMY_API_KEY?.trim() ?? "";
var ALCHEMY_GAS_POLICY_ID = publicEnv?.NEXT_PUBLIC_ALCHEMY_GAS_POLICY_ID?.trim();
var PIMLICO_API_KEY = publicEnv?.NEXT_PUBLIC_PIMLICO_API_KEY?.trim() ?? "";
var AA_PROVIDER_OVERRIDE = publicEnv?.NEXT_PUBLIC_AA_PROVIDER?.trim().toLowerCase();
function resolveAAProvider(preference = "auto") {
  if (preference === "alchemy" || preference === "pimlico") {
    return preference;
  }
  if (AA_PROVIDER_OVERRIDE === "alchemy" || AA_PROVIDER_OVERRIDE === "pimlico") {
...
async function resolveAAProviderState({
  ...
  let fallbackReason;
  if (requestedMode === "7702" && (ownerStrategy.kind === "external-wallet" || shouldUseExternalSigner)) {
    resolvedMode = "4337";
  ...
  if (!provider) {
    return {
      ...
      resolvedMode,
      fallbackReason: fallbackReason ?? "aa_provider_not_configured_fallback_eoa"
    };
  }
  ...
  if (ownerStrategy.kind === "external-wallet" && !canUseExternalWalletOwner) {
    return {
      providerState: { resolved: null, pending: false, error: null },
      resolvedMode,
      fallbackReason: fallbackReason ?? "external_wallet_unavailable_fallback_eoa"
    };
  }
  if (ownerStrategy.kind === "provider-session" && !ownerStrategy.session && !canUseExternalWalletOwner) {
    return {
      providerState: { resolved: null, pending: false, error: null },
      resolvedMode,
  ...
  const chain = chainId ? chainsById[chainId] : void 0;
  if (!chainId || !chain) {
    return {
      providerState: { resolved: null, pending: false, error: null },
      resolvedMode,
  ...
  const apiKey = provider === "alchemy" ? ALCHEMY_API_KEY || void 0 : PIMLICO_API_KEY || void 0;
  if (!apiKey) {
    return {
      providerState: { resolved: null, pending: false, error: null },
      resolvedMode,
      fallbackReason: fallbackReason ?? `aa_${provider}_api_key_missing_fallback_eoa`
    };
  }
  const ownerInput = canUseExternalWalletOwner ? {
    kind: "external-wallet",
    walletClient,
    address
  } : {
    kind: "provider-session",
    provider: ownerStrategy.kind === "provider-session" ? ownerStrategy.provider : "provider",
    session: ownerStrategy.kind === "provider-session" ? ownerStrategy.session : null,
    address
  };
  try {
    const state = await createAAProviderState({
      ...
      mode: resolvedMode,
      apiKey,
      gasPolicyId: provider === "alchemy" ? ALCHEMY_GAS_POLICY_ID : void 0,
      sponsored
    });
    if (!state.account || state.error) {
      ...
      return {
        providerState: { resolved: null, pending: false, error: null },
        resolvedMode,
        fallbackReason: fallbackReason ?? `aa_${provider}_account_unavailable_fallback_eoa`
      };
    }
    ...
    return {
      providerState: { resolved: null, pending: false, error: null },
      resolvedMode,
      fallbackReason: fallbackReason ?? `aa_${provider}_initialization_failed_fallback_eoa`
    };
  }
}
function resolveAASponsorship(providerPreference = "auto") {
  ...
  if (aaProvider === "alchemy") {
    return {
      ...
      sponsorAccount: ALCHEMY_GAS_POLICY_ID || void 0
    };
  }
...
function buildEvmExecutionRuntime(evm, overrides) {
  ...
  const shouldUseExternalSignerForAA = runtime.aaOwner === "provider-session" ? false : runtime.aaOwner === "external-wallet" ? true : runtime.shouldUseExternalSigner;
  return {
    ...runtime,
    sendTransaction: runtime.sendTransaction ?? (runtime.sendTransactionAsync ? async (payload) => executeWalletKitTransaction({
      payload,
      state: {
        currentChainId: runtime.currentChainId,
        capabilities: runtime.capabilities,
        nativeWalletExecution: runtime.nativeWalletExecution,
        sendCallsSyncAsync: runtime.sendCallsSyncAsync ? async (args) => runtime.sendCallsSyncAsync({
          ...args,
          connector: runtime.activeConnector
        }) : void 0,
        sendTransactionAsync: async (args) => runtime.sendTransactionAsync({
          ...args,
          connector: runtime.activeConnector
        }),
        switchChainAsync: runtime.switchChainAsync ? async ({ chainId }) => runtime.switchChainAsync({
          chainId,
          connector: runtime.activeConnector
        }) : void 0,
      ...
      resolveAAProviderState: runtime.resolveAAProviderState ? async (params) => runtime.resolveAAProviderState(params, {
        address: evm.identity(Date.now()).address,
        walletClient: shouldUseExternalSignerForAA ? await runtime.getWalletClientFor({
          connector: runtime.activeConnector,
    ...
    signTypedData: runtime.signTypedData ?? (runtime.signTypedDataAsync ? async (payload) => {
    ...
    signMessage: runtime.signMessage ?? (runtime.signMessageAsync ? async (payload) => {
...
function buildDefaultSvmNetworkOptions(options) {
  return [
    {
      id: "solana-mainnet",
      label: "Solana Mainnet",
      cluster: "solana:mainnet",
      rpcHttpUrl: options?.mainnetRpcHttpUrl ?? DEFAULT_SVM_RPC_HTTP_URLS["solana:mainnet"],
    ...
    },
    {
      ...
      rpcHttpUrl: options?.devnetRpcHttpUrl ?? DEFAULT_SVM_RPC_HTTP_URLS["solana:devnet"],
    ...
    },
    {
      ...
      rpcHttpUrl: options?.testnetRpcHttpUrl ?? DEFAULT_SVM_RPC_HTTP_URLS["solana:testnet"],
...
function AomiWalletNetworkPreferencesProvider({
  ...
  const selectedSolanaNetwork = useMemo4(
...
import { useCallback as useCallback2, useEffect as useEffect8, useMemo as useMemo8, useRef as useRef5 } from "react";
...
import { useEffect as useEffect6, useMemo as useMemo5, useRef as useRef3, useSyncExternalStore } from "react";
...
function useWalletRegistry(opts) {
  const executorsRef = useRef3(opts.executors);
  useEffect6(() => {
  ...
  useEffect6(() => () => store.dispose(), [store]);
...
import { useEffect as useEffect7, useMemo as useMemo7, useRef as useRef4 } from "react";
...
function useWagmiRegistrySource(store) {
  ...
  const connectorKey = useMemo7(
    () => connectors.map((connector) => `${connector.uid}:${connector.id}`).sort().join("|"),
    [connectors]
  );
  const chainKey = useMemo7(
    () => wagmiConfig.chains.map((chain) => chain.id).join("|"),
    [wagmiConfig.chains]
  );
  const previousConfigKeyRef = useRef4(null);
  const previousConnectionsKeyRef = useRef4(null);
  const previousBrandsKeyRef = useRef4(null);
  useEffect7(() => {
...
function useEvmWalletRuntime({
  ...
  const wagmiConfig = useSafeWagmiConfig();
  const registryExecutors = useMemo8(
    () => ({
  ...
  const activeConnector = useMemo8(() => {
  ...
  const evmSwitchInFlightRef = useRef5(false);
  useEffect8(() => {
  ...
  const evmWalletOptions = useMemo8(
    () => dedupeWalletOptions(
      evmConnectors.map(
...
import { useCallback as useCallback3, useEffect as useEffect10, useMemo as useMemo10, useRef as useRef7 } from "react";
import { useWallet as useSolanaWallet } from "@solana/wallet-adapter-react";

// src/lib/wallet-kit/runtime/svm/registry-source.ts
import { useEffect as useEffect9, useMemo as useMemo9, useRef as useRef6 } from "react";
function useSvmRegistrySource(store, opts) {
  const snapshotKey = useMemo9(
    () => `${opts.svmWallet.publicKey ?? ""}:${opts.svmWallet.walletName ?? ""}`,
    [opts.svmWallet.publicKey, opts.svmWallet.walletName]
  );
  const previousKeyRef = useRef6(null);
  useEffect9(() => {
...
async function refreshBlockhash(tx, connection) {
...
var AOMI_DEFAULT_WC_PROJECT_ID = publicEnv?.NEXT_PUBLIC_AOMI_WC_PROJECT_ID ?? publicEnv?.NEXT_PUBLIC_WALLETCONNECT_PROJECT_ID ?? publicEnv?.NEXT_PUBLIC_PROJECT_ID;
...
function warnDuplicateWalletConnect() {
  if (publicEnv?.NODE_ENV === "production") return;
  console.warn(
...
//# sourceMappingURL=chunk-3GP4B2BE.js.map