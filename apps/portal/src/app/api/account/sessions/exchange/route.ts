...
export async function POST(request: NextRequest) {
  try {
    ...
    const response = await fetch(
      `${backendUrl()}/api/_internal/account/sessions/exchange`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "X-Internal-Shared": internalSharedToken(),
        },
...
function backendUrl(): string {
  return (
    env("AOMI_BACKEND_URL") ??
    env("NEXT_PUBLIC_BACKEND_URL") ??
    "http://127.0.0.1:8080"
  ).replace(/\/+$/, "");
}

function internalSharedToken(): string {
  const token = env("INTERNAL_SHARED_TOKEN");
  if (!token) {
    throw new HttpError(503, "Internal backend exchange is not configured");
  }
  return token;
}
