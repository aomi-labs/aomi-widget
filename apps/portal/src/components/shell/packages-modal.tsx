...
export function PackagesModal({ onClose }: PackagesModalProps) {
  ...
  const [resourceType, setResourceType] = useState<"apps" | "skills">("apps");
    ...
    >
      ...
      >
        <header className="border-aomi-border relative shrink-0 border-b px-[22px] pb-2 pt-[22px]">
          <h1 id="library-title" className="text-[15px] font-semibold">
            Library
          </h1>
          <button
            type="button"
        ...
        <div className="flex min-h-0 flex-1 flex-col px-[22px] pb-5 pt-4">
          <label className="border-aomi-border bg-aomi-surface focus-within:border-aomi-muted flex h-[38px] items-center gap-2.5 rounded-full border px-4 transition-colors">
            ...
            <input
              ...
              placeholder={
                resourceType === "apps" ? "Search apps" : "Search skills"
              }
          ...
          <div
            ...
            className="border-aomi-border mt-4 flex items-center gap-1 border-b"
          >
            <LibraryTab
              active={resourceType === "apps"}
              icon={AppWindow}
              label="Apps"
              onClick={() => setResourceType("apps")}
            />
            <LibraryTab
              active={resourceType === "skills"}
              icon={WandSparkles}
              label="Skills"
            ...
            <>
              ...
              <div className="mt-3 flex items-center justify-between gap-3">
                <div className="border-aomi-border bg-aomi-surface flex rounded-full border p-[3px]">
                  {(["public", "personal"] as PackageVisibility[]).map(
                    (view) => (
                      <button
                        key={view}
                        type="button"
...
function LibraryTab({
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={`relative flex items-center gap-2 px-3 pb-2.5 text-[13px] transition-colors ${
        active
    ...
    >
      ...
      {label}
      {active ? (
        <span className="bg-aomi-fg absolute inset-x-2 -bottom-px h-0.5 rounded-full" />
      ) : null}
    </button>
  );
}
