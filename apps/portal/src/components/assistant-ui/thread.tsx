...
const extractActionItemPrompt = (text: string): string | null => {
  const matches = [...text.matchAll(/^Action item:\s*(.+)$/gim)];
...
const AssistantMessage: FC = () => {
  ...
  const text = content
    ...
    .join("\n")
    .trim();
  const actionItemPrompt = extractActionItemPrompt(text);
    ...
    <MessagePrimitive.Root asChild>
      ...
      >
        <div className="aui-assistant-message-content text-foreground mx-2 break-words text-sm leading-5">
          ...
          {actionItemPrompt ? (
            <div className="mt-3">
              <ThreadPrimitive.Suggestion prompt={actionItemPrompt} send asChild>
                <Button
                  variant="outline"
                  size="sm"
                  className="rounded-full"
                  aria-label={actionItemPrompt}
                >
                  {actionItemPrompt}
                </Button>
              </ThreadPrimitive.Suggestion>
