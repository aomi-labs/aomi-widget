...
const WIRE: WireModelStatement = {
  ...
  payment: [
    { method: "null", credits_used: 100, usd: 1.0, paid_credits: 0, paid_usd: 0 },
    { method: "coinbase", credits_used: 20, usd: 0.2, paid_credits: 20, paid_usd: 0.2 },
    { method: "byok", credits_used: 30, usd: 0.3, paid_credits: 0, paid_usd: 0 },
  ],
...
describe("statement adapter", () => {
  ...
  it("maps the wire statement onto MonthlyStatement with tool/outcome absent", () => {
    const month = toMonthlyStatement(WIRE, "2026-07", { included: 500, used: 120 });
    ...
    expect(month.apps.every((a) => a.tool === null && a.outcome === null)).toBe(true);
    expect(month.byApp.every((r) => r.toolUsd === null && r.outcomeUsd === null)).toBe(
      true,
    );
    ...
    expect(month.payment.allowanceCredits).toEqual({ included: 500, used: 120 });
  });
});
