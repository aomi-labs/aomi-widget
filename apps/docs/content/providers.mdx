...
import { AomiRuntimeProvider, ThreadContextProvider } from "@aomi-labs/react";
import {
  Thread,
  ThreadListSidebar,
  SidebarProvider,
...
`AomiRuntimeProvider` calls the backend defined in `packages/react/src/api/client.ts`:

- `GET /api/state?session_id=` → `{ messages, is_processing, session_exists, pending_wallet_tx }`
...
Shape responses to `SessionMessage` in `packages/react/src/api/types.ts`; the runtime converts them into `@assistant-ui/react` message objects and updates the current thread.
