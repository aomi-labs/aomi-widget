...
const TTL_SECONDS = Number(process.env.AOMI_BFF_ACCOUNT_BEARER_TTL_SECONDS ?? "300");
...
function privateKeyPem(): string {
  ...
  if (!pem) {
    throw new Error("AOMI_BFF_PRIVATE_KEY_PEM is required to mint AccountBearers");
  }
  return pem.replace(/\\n/g, "\n");
}
